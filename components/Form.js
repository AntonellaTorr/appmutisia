...
export default function Form() {
  ...
  const [detalle, setDetalle] = useState(''); // Para el detalle
const [imagen, setImagen] = useState(''); // Para la URL de la imagen
  ...
  const onPress = async () => {
    try {
      const data = {
        ...
        detalle: detalle || undefined, // Si el detalle está vacío, envía undefined
        image: imagen || undefined,   // Si la imagen está vacía, envía undefined
      ...
      navigation.navigate('cervezaLista', { reloadData: true });

      setNombre('');
      setAmargor(null);
      setGraduacion('');
      setDetalle(''); // Limpiar detalle
