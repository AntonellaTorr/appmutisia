...
// Servir la carpeta 'assets' como archivos estáticos, esto es para las imagenes
...
app.get("/api/cervezas/paginadas", (req, res) => {
    ...
    const limit = parseInt(req.query.limit) || 10; // Cantidad de resultados por pagina (por defecto 10)
    ...
    const totalItems = cervezas.length; //esto era lo q nos faltaba agregar

    // Meta información
    const totalPages = Math.ceil(cervezas.length / limit);

    //enviamos los resultados junto con info de la paginacion
...
*/
// Metodo GET para obtener una cerveza por su codigo
...
// Metodo POST para agregar una nueva cerveza
...
// Metodo PUT para actualizar una cerveza existente
app.put("/api/cervezas/:codigo", (req, res) => {
    ...
    // validamos los datos de actualizacion
    const { error } = validarCerveza(req.body);
    ...
    // enviamos la cerveza validadad como respuesta
    res.send(cerveza);
});

// validamos la cerveza usando joi
